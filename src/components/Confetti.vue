...
<script>
  import confetti from "./../assets/confetti_sprites.png";
  export default {
    name: 'Confetti',
    mounted(){
      ...
      function setupParticles( aSpriteSheetImage )
      {
        ...
        const particleSpriteObject = {
          ...
          frameHeight:100,
          colorSheets:[],
          spriteLocationMap:[{x:0,y:0},{x:1000,y:0},{x:200,y:0},{x:300,y:0},{x:400,y:0},{x:500,y:0},{x:600,y:0},{x:700,y:0}]
        };
        ...
        particles.config.forEach( function( config, index ){
          ...
          plane.sort(function(a,b){ return a.scale - b.scale; });
      ...
      function setupRequestAnimationFrame( aFallbackFrameRate )
      {
        ...
        else
        {
          ...
          window.cancelAnimationFrame = function( anId ){ clearTimeout( anId ); };
        }
      }
